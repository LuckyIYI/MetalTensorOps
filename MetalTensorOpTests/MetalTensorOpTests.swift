...
struct Metal4Tests {
    @Test func testSimdgroupMatmul() async throws {
...
struct Metal3Tests {
    @Test func simdgroupMatmulMetal3() async throws {
        guard let device = MTLCreateSystemDefaultDevice() else {
            throw TestError("Metal is not supported on this device")
        }
        ...
        // Compile the legacy kernel
        let library = try device.makeDefaultLibrary(bundle: .main)
        let encoder = try MatMulMetal3Encoder(device: device, library: library, bufferA: bufferA, bufferB: bufferB, bufferC: bufferC)
